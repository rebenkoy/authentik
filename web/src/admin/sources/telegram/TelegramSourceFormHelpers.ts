...
import { TelegramSourcePropertyMapping, PropertymappingsApi } from "@goauthentik/api";
...
export function propertyMappingsSelector(object: string, instanceMappings?: string[]) {
    if (!instanceMappings) {
        return async (mappings: DualSelectPair<TelegramSourcePropertyMapping>[]) =>
            mappings.filter(
                ([_0, _1, _2, mapping]: DualSelectPair<TelegramSourcePropertyMapping>) =>
                    object == "user" &&
                    mapping?.managed?.startsWith("goauthentik.io/sources/telegram/user/default/"),
            );
    }
