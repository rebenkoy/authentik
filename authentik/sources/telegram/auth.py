"""authentik Telegram Authentication Backend"""
...
class TelegramBackend(InbuiltBackend):
    """Authenticate users against Telegram realm"""

    def authenticate(self, request: HttpRequest, **kwargs):
        """Try to authenticate a user via telegram"""
        ...
        if user:
            self.set_method("telegram", request, source=source)
            return user
        return None

    def auth_user(
        self, request: HttpRequest, username: str, realm: str | None, password: str, **filters
    ) -> tuple[User | None, TelegramSource | None]:
        sources = TelegramSource.objects.filter(enabled=True)
        ...
        if user is not None:
            ...
            user_source_connections = UserTelegramSourceConnection.objects.filter(
        ...
        # no realm specified, we can't do anything
            ...
            user_source_connections = UserTelegramSourceConnection.objects.none()

        if not user_source_connections.exists():
            LOGGER.debug("no telegram source found for user", username=username)
            return None, None

        for user_source_connection in user_source_connections.prefetch_related().select_related(
            "source__telegramsource"
        ):
            # User either has an unusable password,
            # or has a password, but couldn't be authenticated by ModelBackend
            # This means we check with a kinit to see if the Telegram password has changed
            if self.auth_user_by_kinit(user_source_connection, password):
                # Password was successful in kinit to Telegram, so we save it in database
                if (
                    user_source_connection.source.telegramsource.password_login_update_internal_password
                ):
                    LOGGER.debug(
        ...
        LOGGER.debug("no valid telegram source found for user", user=user)
        return None, None

    def auth_user_by_kinit(
        self, user_source_connection: UserTelegramSourceConnection, password: str
    ) -> bool:
